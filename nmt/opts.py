import argparse
from csnli.nmt.onmt.modules.SRU import CheckSRU
...
def translate_opts(parser):
    ...
    group.add_argument('-src', default=None,
    ...
    group.add_argument('-lang', choices=['hin', 'eng'],
                       help='Source language (only in case of transliteration)')
    ...
    group.add_argument('--dynet-seed', dest='seed', type=int, default=127)
    group.add_argument('--train', help='CONLL/TNT Train file')
    group.add_argument('--dev', help='CONLL/TNT Dev/Test file')
    group.add_argument('--test', dest='tfile', help='Raw Test file')
    group.add_argument('--eng-pretrained-embd', dest='eembd', help='Pretrained word2vec Embeddings')
    group.add_argument('--hin-pretrained-embd', dest='hembd', help='Pretrained word2vec Embeddings')
    group.add_argument('--elimit', type=int, default=None, help='load only top-n pretrained English word vectors (default=all vectors recommended=400k)')
    group.add_argument('--hlimit', type=int, default=None, help='load only top-n pretrained Hindi word vectors (default=all vectors recommended=200k)')
    group.add_argument('--trainer', default='momsgd', help='Trainer [cysgd|momsgd|adam|adadelta|adagrad|amsgrad]')
    group.add_argument('--activation-fn', dest='act_fn', default='tanh', help='Activation function [tanh|relu|sigmoid]')
    group.add_argument('--iter', type=int, default=100, help='No. of Epochs')
    group.add_argument('--bvec', type=int, help='1 if binary embedding file else 0')
    group.add_argument('--etrans', help='OpenNMT English Transliteration Model')
    group.add_argument('--htrans', help='OpenNMT Hindi Transliteration Model')
    ...
    group.add_argument('--retune-model', dest='retune_model', help='Retune Pretrained Model')
    group.add_argument('--output-file', dest='ofile', default='/tmp/out.txt', help='Output File')
    group.add_argument('--daemonize', dest='isDaemon', action='store_true', default=False, help='Daemonize parser')
    group.add_argument('--port', type=int, dest='daemonPort', help='Specify a port number')
    group.add_argument('--test-file', dest='tfile', help='Raw Test file')
    group.add_argument('--lid-model', dest='lid_model', help='Load Pretrained Model')
    group.add_argument('--wx', action='store_true', help='set this flag to return Hindi words in WX')
